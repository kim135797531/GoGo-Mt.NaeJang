...
public class BoardWriteActivity extends Activity {
		...
		communityWriteSendButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (document.nickName.length() == 0) {
					Toast.makeText(BoardWriteActivity.this, "닉네임을 입력해 주세요.",
							Toast.LENGTH_LONG).show();
					return;
				} else if (document.nickName.length() > 10) {
					Toast.makeText(BoardWriteActivity.this,
							"닉네임은 10자 이내이어야 합니다.", Toast.LENGTH_LONG).show();
					return;
				} else if (document.password.length() == 0) {
					Toast.makeText(BoardWriteActivity.this, "비밀번호를 입력해 주세요.",
							Toast.LENGTH_LONG).show();
					return;
				} else if (document.password.length() > 20) {
					Toast.makeText(BoardWriteActivity.this,
							"비밀번호는 20자 이내이어야 합니다.", Toast.LENGTH_LONG).show();
					return;
				} else if (document.title.length() == 0) {
					Toast.makeText(BoardWriteActivity.this, "제목을 입력해 주세요.",
							Toast.LENGTH_LONG).show();
					return;
				} else if (document.content.length() == 0) {
					Toast.makeText(BoardWriteActivity.this, "내용을 입력해 주세요.",
							Toast.LENGTH_LONG).show();
					return;
				}
				...
				ManageNetwork.getInst().enhancedSendDocumentFunc(BoardWriteActivity.this, document);
			}
		});
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		Cursor cursor = getContentResolver().query(imageURI, path, null, null,
				null);
		cursor.moveToFirst();
	...
	private Dialog initCategoryDialog() {
		...
		builder.setSingleChoiceItems(categoryList, curCategory,
